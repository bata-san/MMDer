...
import { beginPhysicsPull, enablePhysics, pokePhysics, resetPhysics, updatePhysicsPull, } from './physics.js';
import { camera, canvas, controls, pointer, raycaster, scene, stage } from './scene.js';
import { state } from './state.js';
let pointerId = null;
let dragPlane = new THREE.Plane();
let startPoint = new THREE.Vector3();
let modelDragEntries = [];
let pullHandle = null;
let pullTarget = new THREE.Vector3();
let operation = 'none';
const marker = new THREE.Mesh(new THREE.SphereGeometry(0.075, 16, 12), new THREE.MeshBasicMaterial({ color: 0x3d8df2, transparent: true, opacity: 0.72, depthTest: false }));
...
function pick(event) {
    setPointer(event);
    const hits = raycaster.intersectObjects(state.models.filter((model) => model.visible).map((model) => model.mesh), true);
...
function selectPicked(model, event) {
...
async function pokePicked(hit) {
    if (!hit.model.physics)
        await enablePhysics(hit.model);
    if (!hit.model.physics)
        return;
    const cameraDirection = camera.getWorldDirection(new THREE.Vector3());
    const direction = hit.normal.clone().multiplyScalar(-0.35).add(cameraDirection).normalize();
    const affected = pokePhysics(hit.model, hit.point, direction, state.interactionSettings.pokeStrength, state.interactionSettings.pokeRadius);
    if (affected) {
...
async function beginPull(hit, event) {
    if (!hit.model.physics)
        await enablePhysics(hit.model);
    if (!hit.model.physics)
        return;
...
function prepareManipulationSelection(model, event) {
    const additive = event.ctrlKey || event.metaKey || event.shiftKey;
    if (additive) {
        if (!state.selectedModels.includes(model))
            toggleModelSelection(model, true);
        else
...
function beginModelMove(hit, event) {
...
function onPointerDown(event) {
    if (event.button !== 0)
        return;
    const rigEdit = document.querySelector('#rig-edit');
    if (rigEdit?.checked)
        return;
...
function endOperation(event) {
    if (operation === 'none')
        return;
    if (operation === 'move')
        modelDragEntries.forEach((entry) => resetPhysics(entry.model));
    operation = 'none';
    modelDragEntries = [];
    pullHandle = null;
    marker.visible = false;
...
export function pokeFromRay(sourceRaycaster) {
    const hits = sourceRaycaster.intersectObjects(state.models.filter((model) => model.visible).map((model) => model.mesh), true);
    const hit = hits[0];
    if (!hit)
        return false;
    const model = modelFromObject(hit.object);
