...
export function applyPhysicsSettings(item = state.active) {
    ...
    runtime.engine.bodies.forEach((wrapper) => {
        const body = wrapper.body;
        if (!body)
            return;
    ...
    runtime.engine.constraints?.forEach((wrapper) => configureConstraint(wrapper, stiffness));
}
...
export function disablePhysics(item) {
    if (!item.physics)
        return;
...
export function resetPhysics(item = state.active) {
    if (!item?.physics)
        return;
    item.physics.accumulator = 0;
    item.physics.engine.reset?.();
...
export function resetAllPhysics() {
...
export function stepPhysics(item, delta, time) {
    const runtime = item.physics;
    if (!state.physics || !runtime?.enabled)
        return;
    ...
    if (steps === runtime.maxSubSteps && runtime.accumulator >= runtime.fixedStep)
