...
function configureMmdMaterial(material) { if (!material) return material; if (material.map) material.map.colorSpace = THREE.SRGBColorSpace; if (material.emissiveMap) material.emissiveMap.colorSpace = THREE.SRGBColorSpace; [material.alphaMap, material.normalMap, material.bumpMap, material.aoMap, material.gradientMap].filter(Boolean).forEach(texture => texture.colorSpace = THREE.NoColorSpace); if (material.gradientMap) { material.gradientMap.minFilter = THREE.NearestFilter; material.gradientMap.magFilter = THREE.NearestFilter; material.gradientMap.generateMipmaps = false; material.gradientMap.needsUpdate = true; } material.side = THREE.DoubleSide; material.depthTest = true; material.dithering = true; if (material.map && material.opacity >= .98) { material.transparent = false; material.alphaTest = Math.max(material.alphaTest || 0, .02); material.depthWrite = true; } const outline = material.userData?.outlineParameters; if (outline) { outline.thickness = Math.min(.012, Math.max(0, outline.thickness || 0)); outline.mmdBaseThickness = outline.thickness; outline.visible = outline.visible !== false && outline.thickness > 0; } material.needsUpdate = true; return material; }
