...
import type { PhysicsPart, SceneModel } from './types.js';
...
function partForBody(wrapper: any, item: SceneModel): PhysicsPart {
  ...
  if (/hair|髪|前髪|後髪/.test(name)) return 'hair';
  if (/skirt|cloth|ribbon|dress|スカート|リボン|衣/.test(name)) return 'cloth';
  return 'body';
}

function precision(): { unitStep: number; maxStepNum: number } {
  return [
...
export function applyPhysicsSettings(item: SceneModel | null = state.active): void {
  ...
  item.physics.bodies.forEach((wrapper: any) => {
...
export function applyWind(item: SceneModel, time: number): void {
  if (!item.physics?.bodies || !window.Ammo) return;
  const { wind, turbulence, parts } = state.physicsSettings;
  if (!wind && !turbulence) return;
  const Ammo = (window as any).Ammo;
  item.physics.bodies.forEach((wrapper: any, index: number) => {
    const part = partForBody(wrapper, item);
    if ((part !== 'hair' && part !== 'cloth') || !parts[part]) return;
    const body = wrapper.body;
    if (!body) return;
    const gust = wind + Math.sin(time * 2.1 + index * 1.7) * turbulence * wind;
    const force = new Ammo.btVector3(
      gust,
      Math.sin(time * 3.7 + index) * turbulence * wind * 0.2,
      Math.cos(time * 1.3 + index) * turbulence * wind * 0.35,
    );
