...
function assetRow(asset: StoredAsset, action: () => void): HTMLElement {
...
export function renderLibraries(): void {
  const draw = (
    kind: StoredAsset['kind'], targetSelector: string, countSelector: string,
    emptyMessage: string, action: (asset: StoredAsset) => void,
  ): void => {
...
export function renderSceneModels(): void {
  const root = $('#models');
  root.innerHTML = '';
...
export function renderMorphs(): void {
  const root = $('#morphs');
  ...
  const groups = new Map<string, { mesh: any; index: number }[]>();
  meshes.forEach((mesh: any) => {
    Object.entries(mesh.morphTargetDictionary as Record<string, number>).forEach(([name, index]) => {
  ...
  [...groups.entries()].slice(0, 140).forEach(([name, bindings]) => {
    ...
    const range = row.querySelector<HTMLInputElement>('input')!;
    const valueOutput = row.querySelector<HTMLOutputElement>('output')!;
    range.oninput = () => {
...
export function renderMaterials(): void {
  ...
  state.active.mesh.traverse((mesh: any) => {
    ...
    (Array.isArray(mesh.material) ? mesh.material : [mesh.material]).forEach((material: any) => entries.push(material));
  });
  entries.forEach((material, index) => {
    ...
    row.querySelector<HTMLInputElement>('input')!.onchange = (event) => {
      material.visible = (event.currentTarget as HTMLInputElement).checked;
    };
    root.append(row);
  });
  if (!entries.length) root.innerHTML = '<div class="empty">マテリアルが見つかりません</div>';
}

export function renderActivePanels(): void {
  renderSceneModels();
  renderMorphs();
  renderMaterials();
}

export function showAllMaterials(): void {
  state.active?.mesh.traverse((mesh: any) => {
    if (!mesh.isMesh || !mesh.material) return;
    (Array.isArray(mesh.material) ? mesh.material : [mesh.material]).forEach((material: any) => { material.visible = true; });
  });
  renderMaterials();
}
