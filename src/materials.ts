...
function rememberNativeState(material: any): NativeMaterialState {
  if (material.userData.mmdLabNativeMaterial) return material.userData.mmdLabNativeMaterial;
  const outline = material.userData?.outlineParameters;
  const native: NativeMaterialState = {
...
function applyNativeMaterialSettings(material: any): void {
  ...
  if (outline) {
    ...
    if (native.outlineColor) outline.color = [...native.outlineColor];
  }
}
