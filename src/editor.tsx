...
export function Editor() {
  ...
  return <main className="grid h-screen grid-rows-[48px_minmax(0,1fr)] bg-background text-foreground" style={{ gridTemplateColumns: grid }}>
    ...
    <aside className="overflow-hidden border-l"><Tabs defaultValue="view" className="h-full"><TabsList className="h-11 w-full justify-start gap-1 overflow-x-auto rounded-none border-b bg-transparent p-1">{tabs.map(([id, label, Icon]) => <TabsTrigger key={id} value={id} title={label}><Icon className="size-3.5" /><span>{label}</span></TabsTrigger>)}</TabsList>
