...
import { arrangeModels, clearModelSelection, clearModels, loadModel, selectAllModels } from './models';
import { resetAllPhysics } from './physics';
import { BUILD_VERSION, state } from './state';
import { grid, renderer } from './scene';
...
export function Editor() {
  ...
  return <main className="grid h-screen grid-rows-[48px_minmax(0,1fr)] bg-background text-foreground" style={{ gridTemplateColumns: grid }}>
    ...
    <aside className="min-w-0 overflow-hidden border-l"><Tabs defaultValue="view" className="h-full"><TabsList className="grid h-12 w-full grid-cols-5 gap-0 rounded-none border-b bg-transparent p-1">{tabs.map(([id, label, Icon]) => <TabsTrigger key={id} value={id} title={label} aria-label={label} className="min-w-0 px-0"><Icon className="size-4" /><span className="sr-only">{label}</span></TabsTrigger>)}</TabsList>
      <TabsContent value="view" className="space-y-5 p-4"><p className="text-sm font-medium">表示</p><label className="flex items-center justify-between text-xs">グリッド <Switch defaultChecked onCheckedChange={(enabled) => { grid.visible = enabled; }} /></label><label className="flex items-center justify-between text-xs">影 <Switch defaultChecked onCheckedChange={(enabled) => { renderer.shadowMap.enabled = enabled; }} /></label><label className="flex items-center justify-between text-xs">輪郭線 <Switch defaultChecked onCheckedChange={(enabled) => { state.outline = enabled; }} /></label></TabsContent>
