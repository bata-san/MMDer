...
export async function loadBuiltInBackground(): Promise<boolean> {
  ...
  if (!BACKGROUND_GZIP_BASE64_CHUNKS.length) {
    console.info('Exact built-in background is not generated; using the simple stage.');
    return false;
  }
  ...
  loadingBackground = (async () => {
    ...
    try {
      ...
      grid.visible = false;
      console.info(
        `Loaded exact background ${BACKGROUND_NAME} (${BACKGROUND_UNCOMPRESSED_BYTES.toLocaleString()} bytes, ${BACKGROUND_SHA256}).`,
      );
